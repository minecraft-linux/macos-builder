...
try:
	makedirs('empty')
	call(['hdiutil', 'create', '-fs', 'HFS+', '-format', 'UDRW', '-sectors', str(image_sectors), '-volname', VOL_NAME, '-srcfolder', 'empty', '-quiet', DMG_OUTPUT_PATH])
finally:
	rmdir('empty')
...
call(['hdiutil', 'convert', '-format', 'UDZO', '-imagekey', 'zlib-level=6', '-o', DMG_OUTPUT_PATH, '-quiet', DMG_OUTPUT_PATH + '.tmp'])
